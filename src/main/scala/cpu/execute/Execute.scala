...
class Execute(implicit c: Config = DefCon) extends MultiIOModule {
  ...
  decode.wdata       := MuxLookup(
    memory.sel_reg_wdata,
    0.U,
    Array(SEL_REG_WDATA_EX -> memory.alu_out, SEL_REG_WDATA_LNK -> memory.pcp8),
  )
  writeback.c0_raddr := memory.c0.waddr // 都是rd
  hazard.c0_raddr    := writeback.c0_raddr

  private def c0 = MuxLookup(
    hazard.forward_c0,
    writeback.c0_rdata,
    Array(FORWARD_C0_MEM -> memory.fwd_c0.wdata, FORWARD_HILO_WB -> writeback.c0_data),
  )

  // misc output
  memory.hi.wdata    := MuxCase(
    num1,
    Array( // 默认num1是mthi, rs读出来的值 -- 要加上em.hi_wen做条件限定么？
