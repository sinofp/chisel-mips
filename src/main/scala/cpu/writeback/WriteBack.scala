...
//  val mem_rdata = Wire(UInt(32.W))
  ...
  decode.wdata := MuxLookup(
    sel_reg_wdata,
    0.U,
    Array(SEL_REG_WDATA_EX -> alu_out, SEL_REG_WDATA_LNK -> pcp8, SEL_REG_WDATA_MEM -> memory.mem_rdata),
  )
  ...
  if (c.oTeachSoc) {
    ...
    debug_wb.get.getElements.reverse.zipWithIndex.foreach { case (sink, n) =>
      BoringUtils.addSource(sink, s"debugwb$n")
    }
  }
}
