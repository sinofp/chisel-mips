...
class Memory(implicit c: Config = DefCon) extends MultiIOModule {
  ...
  locally {
    ...
    en                  := true.B
    wen                 := Mux(
      writeback.except_type === 0.U,
      Mux(mem_wen, MuxLookup(mem_size, "b1111".U, Array(MEM_H -> "b0011".U, MEM_B -> "b0001".U)), 0.U),
      0.U,
    )
    addr                := writeback.alu_out
    wdata               := mem_wdata
    writeback.mem_rdata := MuxLookup(
      mem_size,
      rdata,
      Array(
  ...
  decode.wdata         := MuxLookup(
    0.U,
    writeback.sel_reg_wdata,
    Array(SEL_REG_WDATA_EX -> writeback.alu_out, SEL_REG_WDATA_LNK -> writeback.pcp8),
  )
  ...
  val Cause  = Mux(
    writeback.fwd_c0.wen && writeback.fwd_c0.waddr === CP0_CAUSE,
    writeback.c0_cause.asTypeOf(new Cause).update(writeback.fwd_c0.wdata),
    writeback.c0_cause,
  )
  val EPC    = Mux(writeback.fwd_c0.wen && writeback.fwd_c0.waddr === CP0_EPC, writeback.fwd_c0.wdata, writeback.c0_epc)
  val Status = Mux(
    writeback.fwd_c0.wen && writeback.fwd_c0.waddr === CP0_STATUS,
    writeback.c0_status.asTypeOf(new Status).update(writeback.fwd_c0.wdata),
    writeback.c0_status,
  )
  // exception
  writeback.except_type := MuxCase(
    0.U,
    Array(
