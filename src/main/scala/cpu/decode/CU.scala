...
class CtrlSigs extends Bundle {
  ...
  private def r(alu_fn: B, alu_n: B = 0, check_overflow: B = 0): List[B] = dft(
    alu_fn = alu_fn,
    alu_n = alu_n,
    reg_wen = 1,
    sel_reg_waddr = SEL_REG_WADDR_RD,
    sel_reg_wdata = SEL_REG_WDATA_EX,
    check_overflow = check_overflow,
  )

  private def mf(hi: Boolean = false, lo: Boolean = false, c0: Boolean = false): List[B] = dft(
    reg_wen = 1,
    sel_reg_waddr = if (c0) SEL_REG_WADDR_RT else SEL_REG_WADDR_RD,
    sel_reg_wdata = SEL_REG_WDATA_EX,
    sel_move = if (c0) SEL_MOVE_C0 else if (hi) SEL_MOVE_HI else SEL_MOVE_LO,
  )

  private def mt(hi: Int = 0, lo: Int = 0, c0: Int = 0): List[B] = dft(hi_wen = hi, lo_wen = lo, c0_wen = c0)

  private def l(mem_size: B): List[B] = dft(
    sel_alu2 = SEL_ALU2_IMM,
    sel_imm = SEL_IMM_S,
    alu_fn = FN_ADD,
    reg_wen = 1,
    sel_reg_waddr = SEL_REG_WADDR_RT,
    sel_reg_wdata = SEL_REG_WDATA_MEM,
    mem_size = mem_size,
    load = 1,
    data_sram_en = 1,
  )
  ...
  private def dft(
    sel_alu1: B = SEL_ALU1_RS,
    sel_alu2: B = SEL_ALU2_RT,
    sel_imm: B = SEL_IMM_U,
    alu_fn: B = FN_X,
    mul: B = 0,
    div: B = 0,
    mem_wen: B = 0,
    reg_wen: B = 0,
    sel_reg_waddr: B = SEL_REG_WADDR_RD,
    sel_reg_wdata: B = SEL_REG_WDATA_EX,
    br_type: B = BR_TYPE_NO,
    mem_size: B = MEM_W,
    load: B = 0,
    hi_wen: B = 0,
    lo_wen: B = 0,
  ...
  private def m(alu_fn: B, div: Int = 0, mul: Int = 0): List[B] = dft(
    sel_alu2 = SEL_ALU2_IMM,
    sel_imm = SEL_IMM_S,
    alu_fn = alu_fn,
    mul = mul,
  ...
  private def s(mem_size: B): List[B] = dft(
    sel_alu2 = SEL_ALU2_IMM,
    sel_imm = SEL_IMM_S,
    alu_fn = FN_ADD,
    mem_wen = 1,
  ...
  private def sft(alu_fn: B, v: Boolean = false): List[B] = dft(
    sel_alu1 = if (v) SEL_ALU1_RS else SEL_ALU1_SA,
    sel_alu2 = SEL_IMM_SH,
    alu_fn = alu_fn,
    reg_wen = 1,
    sel_reg_waddr = SEL_REG_WADDR_RD,
  ...
  private def b(br_type: B, link: Int = 0): List[B] = dft(
    sel_imm = SEL_IMM_B,
    alu_fn = FN_SLT,
    reg_wen = link,
    sel_reg_waddr = SEL_REG_WADDR_31,
    sel_reg_wdata = SEL_REG_WDATA_LNK,
    br_type = br_type,
  )

  private def j(r: Boolean = false, l: Int = 0): List[B] = dft(
    sel_alu1 = if (r) SEL_ALU1_SA else SEL_ALU1_RS,
    sel_imm = SEL_IMM_J,
    reg_wen = l,
    sel_reg_waddr = SEL_REG_WADDR_31,
    sel_reg_wdata = SEL_REG_WDATA_LNK,
